...
using feathercast::discovery::DisambiguateShortcutName;
using feathercast::discovery::IsHostExecutable;
...
void QueueDiagnostic(std::wstring category, std::wstring line);
...
class AsyncDiagnostics {
 public:
  ...
  void Enqueue(std::wstring category, std::wstring line) {
    if (!g_diagnosticsEnabled.load(std::memory_order_acquire)) return;
    if (category.empty() || line.empty()) return;
    line.resize(std::min<std::size_t>(line.size(), 2048));
    {
      std::lock_guard lock(mutex_);
      if (stopping_) return;
      if (!worker_.joinable()) {
      ...
      entries_.push_back({std::move(category), std::move(line)});
    }
    wake_.notify_one();
  }
  ...
  void WorkerLoop(std::stop_token token) {
    for (;;) {
      Entry entry;
      {
        ...
        wake_.wait(lock, [&] {
          return stopping_ || token.stop_requested() || !entries_.empty();
        });
        if (entries_.empty() && (stopping_ || token.stop_requested())) return;
        if (entries_.empty()) continue;
        entry = std::move(entries_.front());
        entries_.pop_front();
      }
      if (!g_diagnosticsEnabled.load(std::memory_order_acquire)) continue;
      Append(entry);
    }
  }

  static void Append(const Entry& entry) {
    const auto path = LocalDataPath() / (entry.category + L"-debug.log");
    std::error_code ec;
    if (std::filesystem::file_size(path, ec) > 1024 * 1024) {
      const auto rotated =
          LocalDataPath() / (entry.category + L"-debug.previous.log");
...
void QueueDiagnostic(std::wstring category, std::wstring line) {
  static AsyncDiagnostics diagnostics;
  diagnostics.Enqueue(std::move(category), std::move(line));
}

void DebugGraphicsLog(const std::wstring& line) {
  QueueDiagnostic(L"graphics", line);
}

void DebugPerformanceLog(const std::wstring& line) {
  QueueDiagnostic(L"performance", line);
}
...
bool LoadShortcut(const std::wstring& lnkPath, ShortcutInfo& info) {
  ...
  if (SUCCEEDED(link->GetPath(target, MAX_PATH, &findData, SLGP_RAWPATH))) {
    wchar_t expanded[MAX_PATH]{};
    if (ExpandEnvironmentStringsW(target, expanded, MAX_PATH) > 0) {
      info.target = expanded;
    } else {
      info.target = target;
    }
  }
...
class FeatherCastApp : public feathercast::accessibility::Model {
 public:
  explicit FeatherCastApp(HINSTANCE instance, std::wstring cmdLine)
      : captureEvents_([this] {
        ...
        snippetSaveEvents_([this] {
          NotifyRuntimeEvents();
        }),
        snippetLoadEvents_([this] {
          NotifyRuntimeEvents();
        }),
        ...
        }, [this](feathercast::files::PreparedFileIndex projection) {
          fileProjectionEvents_.Push(std::move(projection));
        }),
        previewEvents_([this] {
          NotifyRuntimeEvents();
        }),
    ...
    performanceGovernor_.Initialize({
        std::max(1u, static_cast<unsigned>(systemInfo.dwNumberOfProcessors)),
  ...
  int Run() {
    ...
    const auto startupPolicy = performanceGovernor_.Policy();
    launchExecutor_.Start(startupPolicy.tier ==
  ...
  bool DispatchQueuedInput() {
    ...
    // Posted runtime notifications are intentionally processed in bounded
    // slices. Prefer already queued user input before taking the next slice,
    // so a background burst cannot sit in front of keyboard or pointer input.
    if (PeekMessageW(&input, nullptr, WM_KEYFIRST, WM_KEYLAST, PM_REMOVE) ||
  ...
  std::vector<feathercast::accessibility::Item> AccessibleItems(HWND hwnd) const override {
    ...
    auto screenRect = [&](RectF rect) {
      const auto physical = feathercast::screenshot::DipToPixel(
          {rect.left, rect.top, rect.right, rect.bottom}, {0, 0}, scale);
    ...
    if (hwnd == captureSelectorHwnd_ && ScreenshotEditorActive()) {
      ...
      const auto layout = ScreenshotToolbarContract(width, height);
      const auto buttons = ScreenshotToolbarItems(width, height);
      const bool parentFocused = GetFocus() == hwnd;
      for (std::size_t index = 0; index < layout.buttons.size(); ++index) {
        ...
        button.screenRect = screenRect({layout.buttons[index].rect.left,
                                        layout.buttons[index].rect.top,
                                        layout.buttons[index].rect.right,
                                        layout.buttons[index].rect.bottom});
      ...
      if (screenshotTextEditing_) {
        ...
        if ((parentFocused && screenshotEditor_.focusIndex ==
                                 static_cast<int>(buttons.size())) ||
            (screenshotTextEditHwnd_ && GetFocus() == screenshotTextEditHwnd_)) {
          text.state |= STATE_SYSTEM_FOCUSED;
        }
        const auto textBounds = ScreenshotTextAnnotationBounds();
        const RectF textRect = ScreenshotLocalRect(textBounds);
      ...
      selection.defaultAction = L"Move or resize selection";
      selection.role = ROLE_SYSTEM_SLIDER;
      selection.state = STATE_SYSTEM_READONLY | STATE_SYSTEM_FOCUSABLE;
      const int selectionIndex = static_cast<int>(layout.buttons.size()) +
                                 (screenshotTextEditing_ ? 1 : 0);
      if (parentFocused && screenshotEditor_.focusIndex == selectionIndex) {
        selection.state |= STATE_SYSTEM_FOCUSED;
      }
      if (const auto current = screenshotEditor_.selection) {
        selection.value = L"x " + std::to_wstring(current->left) + L", y " +
                          std::to_wstring(current->top) + L", " +
                          std::to_wstring(current->Width()) + L" by " +
                          std::to_wstring(current->Height()) + L" pixels";
        selection.screenRect = screenRect(ScreenshotLocalRect(*current));
      } else {
        selection.value = L"No region selected";
        selection.screenRect = screenRect({0, 0, width, height});
      }
      ...
      if (!screenshotStatus_.empty()) {
        ...
        status.screenRect = screenRect({footer.left, footer.top, footer.right,
                                        footer.bottom});
    ...
    if (hwnd == captureSelectorHwnd_) {
      Item selector;
      selector.name = L"Screen region selection";
      selector.description =
          L"Use arrow keys to move. Hold Shift to resize and Enter to confirm. Press Escape to cancel.";
      selector.defaultAction = L"Move or resize selection";
      selector.role = ROLE_SYSTEM_SLIDER;
      selector.state = STATE_SYSTEM_READONLY | STATE_SYSTEM_FOCUSABLE;
      if (GetFocus() == hwnd) selector.state |= STATE_SYSTEM_FOCUSED;
      if (const auto selection =
              feathercast::ui::CaptureUiController::SelectionRect(
                  captureUiState_)) {
        selector.value = L"x " + std::to_wstring(selection->left) + L", y " +
                         std::to_wstring(selection->top) + L", " +
                         std::to_wstring(selection->Width()) + L" by " +
                         std::to_wstring(selection->Height()) + L" pixels";
        selector.screenRect = {selection->left, selection->top,
                               selection->right, selection->bottom};
      } else {
        selector.value = L"No region selected";
    ...
    for (const HitType type : order) {
      ...
      if (type == HitType::SettingsFilter) {
        ...
        setting.value = settingsState_.filter;
  ...
  int AccessibleFocusedChild(HWND hwnd) const override {
    if (hwnd == captureSelectorHwnd_ && ScreenshotEditorActive()) {
      ...
      const int textChild = static_cast<int>(buttons.size()) + 1;
      if (screenshotTextEditHwnd_ && GetFocus() == screenshotTextEditHwnd_) {
        return textChild;
      }
      const auto focusable = ScreenshotFocusableIndices(
          static_cast<float>(client.right) / scale,
          static_cast<float>(client.bottom) / scale, screenshotTextEditing_);
      const auto focused = std::find(focusable.begin(), focusable.end(),
                                     screenshotEditor_.focusIndex);
      if (GetFocus() != hwnd) return 0;
      if (focused != focusable.end()) {
        const int virtualIndex = screenshotEditor_.focusIndex;
        return virtualIndex + 1;
      }
      return focusable.empty() ? 0 : focusable.front() + 1;
    }
  ...
  void NotifyRuntimeEvents() {
    if (!hwnd_ || stopThreads_) return;
    if (runtimeEventNotificationPending_.exchange(true,
                                                   std::memory_order_acq_rel)) {
      return;
    }
    if (PostMessageW(hwnd_, WM_APP_EVENTS, 0, 0) == FALSE) {
      runtimeEventNotificationPending_.store(false,
  ...
  std::size_t PendingRuntimeEvents() const {
    return captureEvents_.Size() + persistenceEvents_.Size() +
           ...
           iconEvents_.Size() + networkEvents_.Size() + timerEvents_.Size();
  }
    ...
    if (hwnd == captureSelectorHwnd_) {
      if (ScreenshotEditorActive()) {
        ...
        if (child > 0 && child <= static_cast<int>(buttons.size())) {
          const auto& button = buttons[static_cast<size_t>(child - 1)];
          if (!ScreenshotToolbarActionAvailable(button.action)) return;
          SetScreenshotFocusIndex(child - 1);
          ...
          SetFocus(screenshotTextEditHwnd_ ? screenshotTextEditHwnd_
                                           : captureSelectorHwnd_);
        } else if (child == static_cast<int>(buttons.size()) +
                              (screenshotTextEditing_ ? 2 : 1)) {
          SetScreenshotFocusIndex(static_cast<int>(buttons.size()) +
                                  (screenshotTextEditing_ ? 1 : 0));
          SetFocus(captureSelectorHwnd_);
        }
  ...
  void AccessibleInvokeChild(HWND hwnd, int child) override {
    if (hwnd == captureSelectorHwnd_) {
      if (ScreenshotEditorActive()) {
        ...
        if (child > 0 && child <= static_cast<int>(buttons.size())) {
          ...
          if (!ScreenshotToolbarActionAvailable(button.action)) return;
          SetScreenshotFocusIndex(child - 1);
          ApplyScreenshotToolbarAction(button.action);
        } else if (screenshotTextEditing_ &&
                   child == static_cast<int>(buttons.size()) + 1) {
          SetScreenshotFocusIndex(static_cast<int>(buttons.size()));
          SetFocus(screenshotTextEditHwnd_ ? screenshotTextEditHwnd_
        ...
        SetFocus(captureSelectorHwnd_);
        if (feathercast::ui::CaptureUiController::ConfirmSelection(
                captureUiState_)) {
    ...
    if (hwnd == hwnd_) {
      ...
      if (child == feathercast::accessibility_projection::SettingsChild()) {
        OpenSettings();
        return;
      }
 ...
 private:
    ...
    switch (msg) {
      ...
      case WM_TIMER:
        if (wParam == TIMER_CLOCK_DISPLAY) {
          ...
          const auto promoted =
              PromotePendingVisibleIcons(performanceGovernor_.Policy()
  ...
  void OnRuntimeEvents() {
    ...
    const auto policy = performanceGovernor_.Policy();
    std::size_t remaining = policy.eventBatch;
    const auto drain = [&remaining](auto& queue, std::size_t queueCap) {
      using Queue = std::remove_reference_t<decltype(queue)>;
      typename Queue::DrainResult result;
      if (remaining == 0) return result;
      result = queue.Drain(std::min(remaining, queueCap));
      remaining -= result.events.size();
      return result;
    };

    // Capture/control events are handled first so recording and screenshot
    // state never waits behind a stale search or icon backlog.
    auto capture = drain(captureEvents_, 8);
    ...
    OnPersistenceEvents(remaining);
    auto snippetSaves = drain(snippetSaveEvents_, 2);
    for (auto& completion : snippetSaves.events) {
      HandleSnippetSaveCompleted(std::move(completion));
    }
    auto snippetLoads = drain(snippetLoadEvents_, 1);
    for (auto& completion : snippetLoads.events) {
      HandleSnippetLoadCompleted(std::move(completion));
    }
    auto clipboard = drain(clipboardEvents_, 2);
    for (auto& observation : clipboard.events) {
      clipboardUpdatePending_.store(false, std::memory_order_release);
      if (observation.excluded || !observation.text ||
    ...
    auto search = drain(searchEvents_, 8);
    for (auto& queued : search.events) {
      std::visit(
    ...
    auto discovery = drain(discoveryEvents_, 4);
    for (auto& result : discovery.events) {
      OnDiscoveryCompleted(std::move(result));
    }
    auto fileIndex = drain(fileIndexEvents_, 4);
    for (auto& status : fileIndex.events) {
      OnFileIndexReady(std::move(status));
    }
    auto projections = drain(fileProjectionEvents_, 2);
    for (auto& projection : projections.events) {
      if (projection.generation != fileIndexProjectionGeneration_ ||
          !projection.files) {
        continue;
      }
      {
        std::lock_guard lock(dataMutex_);
        fileIndex_ = *projection.files;
      }
      fileIndexProjectionPending_ = false;
    ...
    auto previews = drain(previewEvents_, 2);
    for (auto& result : previews.events) {
    ...
    auto decodedIcons = drain(iconEvents_, 8);
    ...
    if (visible_) {
      for (auto& icon : decodedIcons.events) {
        StorePendingDecodedIcon(std::move(icon));
      }
    }
    if (visible_ && !decodedIcons.events.empty()) {
      if (animating_) {
        ...
        PromotePendingVisibleIcons(policy.iconPromotionsPerTick);
        InvalidateRect(hwnd_, nullptr, FALSE);
        ScheduleIconPromotion();
      }
    }
    auto network = drain(networkEvents_, 4);
    for (auto& queued : network.events) {
      std::visit(
          [this](auto&& event) {
            using Event = std::decay_t<decltype(event)>;
    ...
    const auto pumpMicros = static_cast<std::uint64_t>(
    ...
    if (g_diagnosticsEnabled.load(std::memory_order_acquire)) {
      std::wstring diagnostic =
          ...
          std::to_wstring(presentBackpressure ? 1 : 0);
      if (afterTier != beforeTier) {
        diagnostic += L" tier_change=" +
                      std::to_wstring(static_cast<int>(beforeTier)) + L"->" +
                      std::to_wstring(static_cast<int>(afterTier));
      }
      if (pending != 0) diagnostic += L" deferred=event-batch";
      DebugPerformanceLog(diagnostic);
    }
    presentBackpressureObserved_ = false;
    if (pending != 0 || capture.more || snippetSaves.more ||
        snippetLoads.more || clipboard.more || timers.more || search.more ||
        discovery.more || fileIndex.more || projections.more ||
        fileSearch.more || previews.more || decodedIcons.more || network.more) {
      NotifyRuntimeEvents();
    }
  }
  ...
  void HandleScreenshotEvent(
    ...
    if (event.kind == CaptureEventKind::ScreenshotReady) {
      ...
      if (screenshotCaptureUiExcluded_ && captureSelectorHwnd_) {
        SetWindowDisplayAffinity(captureSelectorHwnd_, WDA_NONE);
        screenshotCaptureUiExcluded_ = false;
      }
  ...
  void OnPersistenceEvents(std::size_t& remaining) {
    if (remaining == 0) return;
    auto persistence = persistenceEvents_.Drain(std::min(remaining, size_t{8}));
    remaining -= persistence.events.size();
    for (auto& queued : persistence.events) {
      std::visit(
          [this](auto&& event) {
            using Event = std::decay_t<decltype(event)>;
  ...
  void BeginScreenshotCapture(
    ...
    DwmFlush();
    screenshotCaptureUiExcluded_ =
        captureSelectorHwnd_ && SetWindowDisplayAffinity(
                                  captureSelectorHwnd_,
  ...
  void ShowScreenshotEditor() {
    ...
    if (EnsureRenderResources()) {
      PrewarmGlassSurface(captureSelectorHwnd_, captureSelectorSurface_);
      PromoteScreenshotDraftBitmap();
    }
    ...
    SetFocus(captureSelectorHwnd_);
    NormalizeScreenshotFocus(true);
    UpdateScreenshotTextEditWindow();
  ...
  void ResetScreenshotEditor(bool hideWindow = true) {
    ...
    DestroyScreenshotTextEdit(false);
    if (screenshotCaptureUiExcluded_ && captureSelectorHwnd_) {
      SetWindowDisplayAffinity(captureSelectorHwnd_, WDA_NONE);
      screenshotCaptureUiExcluded_ = false;
    }
  ...
  void ShowCaptureSelector() {
    ...
    feathercast::ui::CaptureUiController::SetBounds(
        captureUiState_, {left, top, left + width, top + height});
  ...
  void BeginCapture(feathercast::ui::CaptureShortcutTarget target) {
    ...
    const auto selectorBounds =
        CaptureScopeFor(target) == feathercast::capture::CaptureScope::Region
            ? VirtualDesktopBounds()
            : feathercast::capture::PixelRect{};
    if (!feathercast::ui::CaptureUiController::Begin(
            captureUiState_, target,
            {selectorBounds.left, selectorBounds.top, selectorBounds.right,
             selectorBounds.bottom})) {
      ShowTrayNotification(L"FeatherCast Capture",
  ...
  bool ScreenshotToolbarActionAvailable(ScreenshotToolbarAction action) const {
    using Phase = feathercast::screenshot::Phase;
    if (action == ScreenshotToolbarAction::Cancel) return ScreenshotEditorActive();
    ...
    switch (action) {
      case ScreenshotToolbarAction::Undo:
        return !screenshotEditor_.annotations.empty();
      case ScreenshotToolbarAction::Redo:
        return !screenshotEditor_.redo.empty();
      case ScreenshotToolbarAction::Save:
      case ScreenshotToolbarAction::Copy:
        return screenshotEditor_.selection &&
               screenshotEditor_.selection->Width() >= 2 &&
               screenshotEditor_.selection->Height() >= 2;
      default:
        return true;
    }
  }

  std::vector<int> ScreenshotFocusableIndices(float width, float height,
                                              bool textEditing) const {
    const auto buttons = ScreenshotToolbarItems(width, height);
    std::vector<int> indices;
    indices.reserve(buttons.size() + 2);
    for (std::size_t index = 0; index < buttons.size(); ++index) {
      if (ScreenshotToolbarActionAvailable(buttons[index].action)) {
        indices.push_back(static_cast<int>(index));
      }
    }
    if (textEditing) indices.push_back(static_cast<int>(buttons.size()));
    // The selection/crop is always the final interactive editor control.  It
    // remains focusable while the preview is being prepared or selected so
    // keyboard-only users have a stable entry point.
    if (ScreenshotEditorActive()) {
      indices.push_back(static_cast<int>(buttons.size()) +
                        (textEditing ? 1 : 0));
    }
    return indices;
  }

  bool IsScreenshotFocusIndexFocusable(int index) const {
    RECT client{};
    if (!captureSelectorHwnd_ || !GetClientRect(captureSelectorHwnd_, &client)) {
      return false;
    }
    const float scale = GetWindowScale(captureSelectorHwnd_);
    const auto focusable = ScreenshotFocusableIndices(
        static_cast<float>(client.right) / scale,
        static_cast<float>(client.bottom) / scale, screenshotTextEditing_);
    return std::find(focusable.begin(), focusable.end(), index) !=
           focusable.end();
  }

  void SetScreenshotFocusIndex(int index, bool invalidate = true) {
    if (!ScreenshotEditorActive()) return;
    const int previousChild = AccessibleFocusedChild(captureSelectorHwnd_);
    screenshotEditor_.focusIndex = index;
    const int nextChild = AccessibleFocusedChild(captureSelectorHwnd_);
  ...
  void NormalizeScreenshotFocus(bool preferSelection = false) {
    if (!ScreenshotEditorActive()) return;
    RECT client{};
    if (!GetClientRect(captureSelectorHwnd_, &client)) return;
    const float scale = GetWindowScale(captureSelectorHwnd_);
    const auto focusable = ScreenshotFocusableIndices(
        static_cast<float>(client.right) / scale,
        static_cast<float>(client.bottom) / scale, screenshotTextEditing_);
    if (focusable.empty()) return;
    int target = screenshotEditor_.focusIndex;
    if (preferSelection || !IsScreenshotFocusIndexFocusable(target)) {
      target = preferSelection ? focusable.back() : focusable.front();
    }
    if (target != screenshotEditor_.focusIndex) {
      SetScreenshotFocusIndex(target);
    }
  }
  ...
  std::vector<ScreenshotToolbarButton> ScreenshotToolbarItems(
    ...
    buttons.reserve(layout.buttons.size());
    for (const auto& button : layout.buttons) {
      buttons.push_back({{button.rect.left, button.rect.top, button.rect.right,
                          button.rect.bottom},
  ...
  void ApplyScreenshotToolbarAction(ScreenshotToolbarAction action) {
    using namespace feathercast::screenshot;
    if (!ScreenshotToolbarActionAvailable(action)) return;
  ...
  int HitTestScreenshotToolbar(float x, float y) const {
    ...
    GetClientRect(captureSelectorHwnd_, &client);
    const float scale = GetWindowScale(captureSelectorHwnd_);
    const auto layout = ScreenshotToolbarContract(
        static_cast<float>(client.right) / scale,
        static_cast<float>(client.bottom) / scale);
    for (std::size_t index = 0; index < layout.buttons.size(); ++index) {
      if (feathercast::screenshot::ToolbarContains(
              layout.buttons[index], {x, y}) &&
          ScreenshotToolbarActionAvailable(layout.buttons[index].action)) {
  ...
  bool FinishScreenshotTextEditing(bool reportEmpty) {
    ...
    if (screenshotTextEditHwnd_) {
      const int length = GetWindowTextLengthW(screenshotTextEditHwnd_);
      std::wstring value(static_cast<size_t>(std::max(0, length)) + 1,
                         L'\0');
      if (length > 0) {
        const int copied = GetWindowTextW(
            screenshotTextEditHwnd_, value.data(), length + 1);
        value.resize(static_cast<size_t>(std::max(0, copied)));
      } else {
        value.clear();
      }
      screenshotTextBuffer_ = std::move(value);
    }
    ...
    screenshotTextEditing_ = false;
    DestroyScreenshotTextEdit(true);
    screenshotTextBuffer_.clear();
    InvalidateScreenshotPreviewCache();
    RECT client{};
    GetClientRect(captureSelectorHwnd_, &client);
    ...
    if (textButton != buttons.end()) {
      SetScreenshotFocusIndex(static_cast<int>(
  ...
  feathercast::screenshot::Rect ScreenshotTextAnnotationBounds() const {
    using feathercast::screenshot::ClampRect;
    using feathercast::screenshot::Normalize;
    if (!screenshotEditor_.gestureStart) return {};
    const auto& points = screenshotEditor_.gesturePoints;
    feathercast::screenshot::Rect result;
    if (points.size() >= 2) {
      result = Normalize({points.front().x, points.front().y, points.back().x,
                          points.back().y});
    } else {
      const auto start = *screenshotEditor_.gestureStart;
      result = {start.x, start.y, start.x + 220, start.y + 48};
    }
    if (result.Width() < 2 || result.Height() < 2) {
      const auto start = *screenshotEditor_.gestureStart;
      result = {start.x, start.y, start.x + 220, start.y + 48};
    }
    return ClampRect(result, screenshotEditor_.sourceBounds);
  }

  void UpdateScreenshotTextEditWindow(bool activate = false) {
    if (!screenshotTextEditHwnd_ || !screenshotTextEditing_ ||
    ...
    GetClientRect(captureSelectorHwnd_, &client);
    const float scale = GetWindowScale(captureSelectorHwnd_);
    const auto local = ScreenshotLocalRect(ScreenshotTextAnnotationBounds());
    const auto physical = feathercast::screenshot::DipToPixel(
    ...
    SetWindowPos(screenshotTextEditHwnd_, HWND_TOP, target.left, target.top,
                 ...
                 activate ? SWP_SHOWWINDOW : SWP_NOACTIVATE | SWP_SHOWWINDOW);
    if (activate) {
      SetFocus(screenshotTextEditHwnd_);
      SendMessageW(screenshotTextEditHwnd_, EM_SETSEL,
                   static_cast<WPARAM>(-1), static_cast<LPARAM>(-1));
    }
  }

  static LRESULT CALLBACK ScreenshotTextEditSubclassProc(
      HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam, UINT_PTR subclassId,
      DWORD_PTR refData) {
    UNREFERENCED_PARAMETER(subclassId);
    auto* app = reinterpret_cast<FeatherCastApp*>(refData);
    if (app && msg == WM_KEYDOWN) {
      const bool control = (GetKeyState(VK_CONTROL) & 0x8000) != 0;
      if (wParam == VK_ESCAPE) {
        app->CancelScreenshotTextEditing();
        return 0;
      }
      if (wParam == VK_RETURN && !control) {
        app->FinishScreenshotTextEditing(true);
        return 0;
      }
      if (control && (wParam == 'S')) {
        app->FinishScreenshotTextEditing(false);
        app->FinalizeScreenshot(feathercast::screenshot::Destination::File);
        return 0;
      }
    }
    if (app && msg == WM_IME_STARTCOMPOSITION) {
      app->PositionScreenshotImeCandidate(hwnd);
    }
    return DefSubclassProc(hwnd, msg, wParam, lParam);
  }

  void PositionScreenshotImeCandidate(HWND edit) const {
    if (!edit) return;
    HIMC context = ImmGetContext(edit);
    if (!context) return;
    POINT caret{};
    if (const LRESULT position = SendMessageW(edit, EM_POSFROMCHAR, 0, 0);
        position != -1) {
      caret.x = GET_X_LPARAM(position);
      caret.y = GET_Y_LPARAM(position);
    }
    COMPOSITIONFORM composition{CFS_POINT, {caret.x, caret.y}, {}};
    ImmSetCompositionWindow(context, &composition);
    CANDIDATEFORM candidate{0, CFS_CANDIDATEPOS, {caret.x, caret.y}};
    ImmSetCandidateWindow(context, &candidate);
    ImmReleaseContext(edit, context);
  }

  bool BeginScreenshotTextEditing() {
    if (!captureSelectorHwnd_ || screenshotEditor_.phase !=
                                      feathercast::screenshot::Phase::Editing ||
        !screenshotEditor_.gestureStart) {
      return false;
    }
    ...
    if (!screenshotTextEditHwnd_) {
      screenshotTextEditHwnd_ = CreateWindowExW(
          WS_EX_CLIENTEDGE, L"EDIT", L"",
          WS_CHILD | WS_VISIBLE | WS_TABSTOP | ES_MULTILINE | ES_AUTOVSCROLL |
              ES_WANTRETURN,
          0, 0, 1, 1, captureSelectorHwnd_, nullptr,
          instance_, this);
      if (!screenshotTextEditHwnd_) {
        screenshotTextEditing_ = false;
        return false;
      }
      SetWindowSubclass(screenshotTextEditHwnd_,
                        &FeatherCastApp::ScreenshotTextEditSubclassProc, 1,
                        reinterpret_cast<DWORD_PTR>(this));
      SendMessageW(screenshotTextEditHwnd_, EM_SETLIMITTEXT, 8192, 0);
      SendMessageW(screenshotTextEditHwnd_, WM_SETFONT,
                   reinterpret_cast<WPARAM>(GetStockObject(DEFAULT_GUI_FONT)),
                   TRUE);
    }
    SetWindowTextW(screenshotTextEditHwnd_, L"");
    UpdateScreenshotTextEditWindow(true);
    RECT client{};
    GetClientRect(captureSelectorHwnd_, &client);
    const float clientScale = GetWindowScale(captureSelectorHwnd_);
    const auto buttons = ScreenshotToolbarItems(
  ...
  void DestroyScreenshotTextEdit(bool focusParent) {
    HWND edit = screenshotTextEditHwnd_;
    screenshotTextEditHwnd_ = nullptr;
    if (edit) {
      RemoveWindowSubclass(edit, &FeatherCastApp::ScreenshotTextEditSubclassProc,
                           1);
      DestroyWindow(edit);
    }
    if (focusParent && captureSelectorHwnd_) SetFocus(captureSelectorHwnd_);
  }

  void CancelScreenshotTextEditing() {
    if (!screenshotTextEditing_) return;
    screenshotTextEditing_ = false;
    screenshotTextBuffer_.clear();
    feathercast::screenshot::EditorController::CancelGesture(screenshotEditor_);
    DestroyScreenshotTextEdit(true);
    RECT client{};
    GetClientRect(captureSelectorHwnd_, &client);
    const float clientScale = GetWindowScale(captureSelectorHwnd_);
  ...
  RectF ScreenshotLocalRect(feathercast::screenshot::Rect rect) const {
    const float scale = GetWindowScale(captureSelectorHwnd_);
    const auto local = feathercast::screenshot::PixelToDip(
        rect,
        {captureVirtualBounds_.left, captureVirtualBounds_.top}, scale);
    return {local.left, local.top, local.right, local.bottom};
  }
  ...
  D2D1_POINT_2F ScreenshotLocalPoint(
    ...
    const float scale = GetWindowScale(captureSelectorHwnd_);
    const auto local = feathercast::screenshot::PixelToDip(
        point, {captureVirtualBounds_.left, captureVirtualBounds_.top}, scale);
    return D2D1::Point2F(local.x, local.y);
  }
    ...
    switch (annotation.tool) {
      ...
      case Tool::Text:
        if (!annotation.text.empty()) {
          const auto textBounds = AnnotationTextBounds(annotation);
          const RectF textRect = ScreenshotLocalRect(textBounds);
          DrawTextBlock(annotation.text, textRect,
                        ScreenshotAnnotationTextFormat(annotation),
                        ScreenshotD2DColor(annotation.color));
        } else {
          StrokeRound(rect, 4.0f, ScreenshotD2DColor(annotation.color, 0.9f),
                      stroke);
        }
  ...
  ScreenshotGesturePreview() const {
    ...
    preview.fontFamily = screenshotEditor_.fontFamily;
  ...
  std::optional<feathercast::screenshot::Rect>
  ScreenshotSelectionForPainting() const {
    if (screenshotEditor_.selection) return screenshotEditor_.selection;
    if (screenshotEditor_.gesturePoints.size() < 2) return std::nullopt;
    const auto first = screenshotEditor_.gesturePoints.front();
    const auto last = screenshotEditor_.gesturePoints.back();
    return feathercast::screenshot::Normalize(
        {first.x, first.y, last.x, last.y});
  }

  void InvalidateScreenshotPreviewCache() {
    screenshotEffectPreviewBitmap_.Reset();
    screenshotEffectPreviewBounds_ = {};
    screenshotEffectPreviewSelection_.reset();
    screenshotEffectPreviewRevision_ = 0;
    ++screenshotAnnotationRevision_;
  }

  void UpdateScreenshotEffectPreview(
      std::optional<feathercast::screenshot::Rect> selection) {
    using namespace feathercast::screenshot;
    if (!screenshotDraft_ || !screenshotDraft_->Valid() ||
        !captureSelectorSurface_.dc || !selection || selection->Empty()) {
      screenshotEffectPreviewBitmap_.Reset();
      screenshotEffectPreviewSelection_.reset();
      screenshotEffectPreviewRevision_ = screenshotAnnotationRevision_;
      return;
    }
    if (screenshotEffectPreviewRevision_ == screenshotAnnotationRevision_ &&
        screenshotEffectPreviewSelection_ == selection) {
      return;
    }
    screenshotEffectPreviewBitmap_.Reset();
    screenshotEffectPreviewSelection_ = selection;
    ...
    std::vector<Annotation> effects;
    for (const auto& annotation : screenshotEditor_.annotations) {
      if (annotation.tool == Tool::Blur || annotation.tool == Tool::Pixelate) {
        effects.push_back(annotation);
      }
    }
    if (const auto gesture = ScreenshotGesturePreview(); gesture &&
        (gesture->tool == Tool::Blur || gesture->tool == Tool::Pixelate)) {
      effects.push_back(*gesture);
    }
    if (effects.empty()) return;

    Rect region = Normalize(*selection);
    bool hasEffectBounds = false;
    Rect effectBounds{};
    for (const auto& annotation : effects) {
      Rect bounds = Normalize(annotation.bounds);
      bounds.left = std::max(bounds.left, region.left);
      bounds.top = std::max(bounds.top, region.top);
      bounds.right = std::min(bounds.right, region.right);
      bounds.bottom = std::min(bounds.bottom, region.bottom);
      if (bounds.Empty()) continue;
      if (!hasEffectBounds) {
        effectBounds = bounds;
        hasEffectBounds = true;
      } else {
        effectBounds.left = std::min(effectBounds.left, bounds.left);
        effectBounds.top = std::min(effectBounds.top, bounds.top);
        effectBounds.right = std::max(effectBounds.right, bounds.right);
        effectBounds.bottom = std::max(effectBounds.bottom, bounds.bottom);
      }
    }
    if (!hasEffectBounds || effectBounds.Width() < 2 || effectBounds.Height() < 2) {
      return;
    }
    std::wstring error;
    const auto rendered = Render(*screenshotDraft_, effectBounds, effects, &error);
    if (!rendered || !rendered->Valid()) return;
    const auto properties = D2D1::BitmapProperties(D2D1::PixelFormat(
        DXGI_FORMAT_B8G8R8A8_UNORM, D2D1_ALPHA_MODE_PREMULTIPLIED));
    ComPtr<ID2D1Bitmap> bitmap;
    if (FAILED(captureSelectorSurface_.dc->CreateBitmap(
            D2D1::SizeU(rendered->width, rendered->height),
            rendered->pixels.data(), rendered->stride, properties,
            bitmap.GetAddressOf()))) {
      return;
    }
    screenshotEffectPreviewBounds_ = effectBounds;
    screenshotEffectPreviewBitmap_ = std::move(bitmap);
  }
  ...
  void PaintScreenshotEditor() {
    ...
    BeginFrameMeasurement();
    if (!EnsureRenderResources()) {
      ScheduleRenderRecovery(L"screenshot-editor-resources", E_FAIL);
      EndPaint(captureSelectorHwnd_, &paint);
      return;
    }
    const HRESULT surfaceResult =
        CreateGlassSurface(captureSelectorSurface_, captureSelectorHwnd_);
    if (FAILED(surfaceResult)) {
      ScheduleRenderRecovery(L"screenshot-editor-surface", surfaceResult);
      EndPaint(captureSelectorHwnd_, &paint);
      return;
    }
    if (screenshotDraft_ && screenshotDraft_->Valid() &&
        !PromoteScreenshotDraftBitmap()) {
      ScheduleRenderRecovery(L"screenshot-editor-bitmap", E_FAIL);
      EndPaint(captureSelectorHwnd_, &paint);
      return;
    }
    UpdateScreenshotTextEditWindow();
    ...
    const auto paintingSelection = ScreenshotSelectionForPainting();
    UpdateScreenshotEffectPreview(paintingSelection);
    const auto frame = feathercast::ui::RenderTransparentFrame(dc, [&] {
      ...
      if (screenshotDraft_ && screenshotDraft_->Valid()) {
        if (screenshotDraftBitmap_) {
          dc->DrawBitmap(screenshotDraftBitmap_.Get(),
                         ...
                         screenshotEditor_.gesturePoints.size() >= 2
      ...
      if (previewSelection && !previewSelection->Empty() &&
        ...
        if (screenshotEffectPreviewBitmap_) {
          dc->DrawBitmap(
              screenshotEffectPreviewBitmap_.Get(),
              ToD2D(ScreenshotLocalRect(screenshotEffectPreviewBounds_)),
      ...
      if (screenshotTextEditing_ && screenshotEditor_.gestureStart) {
        ...
        preview.bounds = ScreenshotTextAnnotationBounds();
      ...
      const auto toolbar = ScreenshotToolbarContract(width, height);
      const auto buttons = ScreenshotToolbarItems(width, height);
      const RectF toolbarBar{toolbar.bar.left, toolbar.bar.top,
                             toolbar.bar.right, toolbar.bar.bottom};
      FillRound(toolbarBar, 12.0f,
                highContrast_ ? D2DColor(theme_.overlayBackground)
                              : D2D1::ColorF(0.05f, 0.06f, 0.08f, 0.94f));
      StrokeRound(toolbarBar, 12.0f, D2DColor(theme_.border), 1.0f);
      for (std::size_t index = 0; index < buttons.size(); ++index) {
        ...
        const bool focused = !disabled &&
                             GetFocus() == captureSelectorHwnd_ &&
                             static_cast<int>(index) == screenshotEditor_.focusIndex;
      ...
      const std::wstring footer = screenshotStatus_.empty()
                                      ? (screenshotEditor_.phase ==
                                       ...
                                       : screenshotStatus_;
      const RectF footerRect{toolbar.footer.left, toolbar.footer.top,
                             toolbar.footer.right, toolbar.footer.bottom};
      DrawTextBlock(footer, footerRect,
                    centerFormat_.Get(),
                    screenshotEditor_.phase ==
                            feathercast::screenshot::Phase::Editing
                        ? D2DColor(theme_.textPrimary)
  ...
  void PaintCaptureSelector() {
    ...
    const auto frame = feathercast::ui::RenderTransparentFrame(dc, [&] {
      ...
      if (selection) {
        const auto rectDip = feathercast::screenshot::PixelToDip(
            ScreenshotRect({selection->left, selection->top, selection->right,
                            selection->bottom}),
            {captureVirtualBounds_.left, captureVirtualBounds_.top}, scale);
        RectF rect{rectDip.left, rectDip.top, rectDip.right, rectDip.bottom};
      ...
      if (captureUiState_.selectionEnd) {
        const auto pointDip = feathercast::screenshot::PixelToDip(
            feathercast::screenshot::Point{captureUiState_.selectionEnd->x,
                                           captureUiState_.selectionEnd->y},
            {captureVirtualBounds_.left, captureVirtualBounds_.top}, scale);
        const float x = pointDip.x;
        const float y = pointDip.y;
    ...
    switch (msg) {
      ...
      case WM_SETCURSOR: {
        ...
        if (screenshotEditor_.tool == Tool::Select &&
          ...
          else if (handle == Handle::TopLeft || handle == Handle::BottomRight)
            cursor = LoadCursorW(nullptr, IDC_SIZENWSE);
          else if (handle == Handle::TopRight || handle == Handle::BottomLeft)
      ...
      case WM_LBUTTONDOWN: {
        ...
        if (toolbarIndex >= 0) {
          SetScreenshotFocusIndex(toolbarIndex);
          RECT client{};
          GetClientRect(hwnd, &client);
          ApplyScreenshotToolbarAction(
              ScreenshotToolbarItems(static_cast<float>(client.right) / scale,
                                     static_cast<float>(client.bottom) / scale)
                  [static_cast<size_t>(toolbarIndex)]
                  .action);
          return 0;
        }
        if (screenshotTextEditing_) {
          ...
          if (screenshotEditor_.activeHandle) {
            ...
            if (screenshotEditor_.tool == Tool::Text) {
              ...
              if (EditorController::UpdateAnnotation(screenshotEditor_, end)) {
                if (!BeginScreenshotTextEditing()) {
                  EditorController::CancelGesture(screenshotEditor_);
                  screenshotStatus_ = L"The text editor could not start.";
                } else {
                  screenshotStatus_ =
      ...
      case WM_COMMAND: {
        ...
        if (source == screenshotTextEditHwnd_ &&
            HIWORD(wParam) == EN_CHANGE) {
          const int length = GetWindowTextLengthW(source);
          std::wstring value(static_cast<size_t>(std::max(0, length)) + 1,
                             L'\0');
          const int copied = GetWindowTextW(
              source, value.data(), static_cast<int>(value.size()));
          value.resize(static_cast<size_t>(std::max(0, copied)));
          screenshotTextBuffer_ = std::move(value);
          InvalidateRect(hwnd, nullptr, FALSE);
          RECT client{};
          GetClientRect(hwnd, &client);
          NotifyWinEvent(EVENT_OBJECT_VALUECHANGE, hwnd, OBJID_CLIENT,
                         static_cast<LONG>(ScreenshotToolbarItems(
                             static_cast<float>(client.right) / scale,
                             static_cast<float>(client.bottom) / scale)
                                                 .size() +
                                             1));
      ...
      case WM_KEYDOWN: {
        ...
        if (screenshotTextEditing_) return 0;

        if (wParam == VK_LEFT || wParam == VK_RIGHT || wParam == VK_UP ||
            wParam == VK_DOWN) {
          const EditorKey key = wParam == VK_LEFT
                                    ? EditorKey::Left
                                    : wParam == VK_RIGHT
                                          ? EditorKey::Right
                                          : wParam == VK_UP ? EditorKey::Up
                                                            : EditorKey::Down;
          const auto result = EditorController::HandleKeyboard(
              screenshotEditor_, key, shift, control);
          if (result == KeyboardResult::Moved) {
            InvalidateScreenshotPreviewCache();
            InvalidateRect(hwnd, nullptr, FALSE);
            NotifyWinEvent(EVENT_OBJECT_VALUECHANGE, hwnd, OBJID_CLIENT, 1);
          }
        ...
        if (wParam == VK_TAB) {
          const auto focusable = ScreenshotFocusableIndices(
              width, height, screenshotTextEditing_);
          if (!focusable.empty()) {
            const bool reverse = shift;
            const auto current = std::find(focusable.begin(), focusable.end(),
                                           screenshotEditor_.focusIndex);
            std::size_t next = 0;
            if (current == focusable.end()) {
              next = reverse ? focusable.size() - 1 : 0;
            } else {
              const auto offset = static_cast<std::size_t>(
                  std::distance(focusable.begin(), current));
              next = reverse
                         ? (offset == 0 ? focusable.size() - 1 : offset - 1)
                         : (offset + 1) % focusable.size();
            }
            SetScreenshotFocusIndex(focusable[next]);
            SetFocus(hwnd);
          }
          return 0;
        }
        if (wParam == VK_RETURN || wParam == VK_SPACE) {
          ...
          if (screenshotEditor_.focusIndex >= 0 &&
              ...
              ScreenshotToolbarActionAvailable(
                  buttons[static_cast<size_t>(screenshotEditor_.focusIndex)]
                      .action)) {
            ApplyScreenshotToolbarAction(
                buttons[static_cast<size_t>(screenshotEditor_.focusIndex)]
                    .action);
      ...
      case WM_CHAR:
        if (screenshotTextEditing_ && !screenshotTextEditHwnd_ &&
    ...
    switch (msg) {
      ...
      case WM_KEYDOWN:
        ...
        if (wParam == VK_LEFT || wParam == VK_RIGHT || wParam == VK_UP ||
          ...
          using Key = feathercast::ui::CaptureKey;
          const auto key = wParam == VK_LEFT
                               ? Key::Left
                               : wParam == VK_RIGHT
                                     ? Key::Right
                                     : wParam == VK_UP
                                           ...
                                           : wParam == VK_DOWN
                                                 ...
                                                 : Key::Enter;
          const auto result =
              feathercast::ui::CaptureUiController::HandleSelectionKey(
                  captureUiState_, key,
                  (GetKeyState(VK_SHIFT) & 0x8000) != 0,
                  (GetKeyState(VK_CONTROL) & 0x8000) != 0);
          if (result == feathercast::ui::CaptureKeyboardResult::Moved) {
            InvalidateRect(hwnd, nullptr, FALSE);
            NotifyWinEvent(EVENT_OBJECT_VALUECHANGE, hwnd, OBJID_CLIENT, 1);
          } else if (result ==
                     feathercast::ui::CaptureKeyboardResult::Confirmed) {
            CompleteCaptureSelection();
          }
        }
        return 0;
    ...
    switch (msg) {
      ...
      case WM_KEYDOWN:
        ...
        if (wParam == VK_ESCAPE && !recording_) {
          if (SettingsFilterFocused() && !settingsState_.filter.empty()) {
            ClearSettingsFilter();
          } else {
            HideSettings();
          }
        ...
        return 0;
      case WM_CHAR:
        OnSettingsChar(static_cast<wchar_t>(wParam));
        return 0;
      case WM_IME_STARTCOMPOSITION:
        settingsState_.filterImeComposition.clear();
        return 0;
      case WM_IME_COMPOSITION:
        OnSettingsFilterImeComposition(lParam);
        return 0;
      case WM_IME_ENDCOMPOSITION:
        settingsState_.filterImeComposition.clear();
        InvalidateRect(hwnd, nullptr, FALSE);
  ...
  void ApplySurfaceGlass(HWND hwnd, HWND blurHwnd, bool& blurApplied) {
    ...
    if (hwnd == hwnd_) {
      ...
      settingsSurface_.visualStateValid = false;
    } else if (hwnd == volumeHwnd_) {
  ...
  HRESULT EnsureGlassDevice() {
    ...
    if (FAILED(hr)) {
      ...
      d3dDevice_.Reset();
      performanceGovernor_.SetWarpRendering(true);
  ...
  void RecordSuccessfulFrame() {
    ...
    if (activeFrameStartQpc_ != 0 && qpcFrequency_.QuadPart > 0) {
      const auto now = NowQpc();
      lastFrameMicros_ = static_cast<std::uint64_t>(
          std::max<LONGLONG>(0, now - activeFrameStartQpc_) * 1'000'000LL /
          qpcFrequency_.QuadPart);
      activeFrameStartQpc_ = 0;
    }
  }

  void BeginFrameMeasurement() {
    if (qpcFrequency_.QuadPart > 0) activeFrameStartQpc_ = NowQpc();
  }
  ...
  PresentResult PresentSurface(GlassSurface& surface, HWND hwnd,
    ...
    if (result == DXGI_ERROR_WAS_STILL_DRAWING) {
      ...
      presentBackpressureObserved_ = true;
  ...
  HRESULT SetSurfaceVisualState(
    ...
    const bool transformChanged =
        ...
        std::abs(surface.lastVisualTransform._11 - compositionTransform._11) >
            0.0001f ||
        std::abs(surface.lastVisualTransform._12 - compositionTransform._12) >
            0.0001f ||
        std::abs(surface.lastVisualTransform._21 - compositionTransform._21) >
            0.0001f ||
        std::abs(surface.lastVisualTransform._22 - compositionTransform._22) >
            0.0001f ||
        std::abs(surface.lastVisualTransform._31 - compositionTransform._31) >
            0.0001f ||
        std::abs(surface.lastVisualTransform._32 - compositionTransform._32) >
            0.0001f;
    ...
    if (surface.visual3 && opacityChanged) {
      result = surface.visual3->SetOpacity(appliedOpacity);
      if (FAILED(result)) return result;
    }
  ...
  HRESULT ApplySurfaceVisualStates() {
    ...
    if (FAILED(result)) return result;
    const bool changed = overlaySurface_.visualStateChanged ||
  ...
  void SyncSettingsSurfaceAfterNativeMove(HWND hwnd) {
    ...
    if (dcompDevice_ && settingsSurface_.visualStateChanged) {
  ...
  void ResizeGlassSurface(GlassSurface& surface, HWND hwnd, UINT width, UINT height) {
    ...
    if (hwnd == hwnd_ && overlayClosing_) return;
    if (hwnd == settingsHwnd_ && settingsClosing_) return;
    if (hwnd == volumeHwnd_ && volumeClosing_) return;
    width = std::max<UINT>(1, width);
    height = std::max<UINT>(1, height);
    DXGI_SWAP_CHAIN_DESC1 desc{};
    if (SUCCEEDED(surface.swapChain->GetDesc1(&desc))) {
  ...
  void ResetTextFormats() {
    ...
    screenshotAnnotationTextFormat_.Reset();
  ...
  void CreateTextFormat(const TextStyle& style,
                        ...
                        const std::wstring* familySpec = nullptr) {
    const std::wstring& families =
        familySpec && !familySpec->empty() ? *familySpec : theme_.fontFamily;
  ...
  IDWriteTextFormat* ScreenshotAnnotationTextFormat(
      const feathercast::screenshot::Annotation& annotation) {
    if (!dwriteFactory_) return bodyFormat_.Get();
    const float scale = GetWindowScale(captureSelectorHwnd_);
    const std::wstring family = annotation.fontFamily.empty()
                                    ? theme_.fontFamily
                                    : annotation.fontFamily;
    const float size = std::max(12.0f,
                                static_cast<float>(annotation.fontSize)) /
                       std::max(0.01f, scale);
    if (!screenshotAnnotationTextFormat_ ||
        std::abs(screenshotAnnotationTextFormatSize_ - size) > 0.01f ||
        std::abs(screenshotAnnotationTextFormatScale_ - scale) > 0.001f ||
        screenshotAnnotationTextFormatFamily_ != family) {
      CreateTextFormat({size, DWRITE_FONT_WEIGHT_SEMI_BOLD,
                        std::max(14.0f, size * 1.25f)},
                       screenshotAnnotationTextFormat_, &family);
      if (screenshotAnnotationTextFormat_) {
        screenshotAnnotationTextFormat_->SetWordWrapping(
            DWRITE_WORD_WRAPPING_WRAP);
        screenshotAnnotationTextFormat_->SetTextAlignment(
            DWRITE_TEXT_ALIGNMENT_LEADING);
        screenshotAnnotationTextFormat_->SetParagraphAlignment(
            DWRITE_PARAGRAPH_ALIGNMENT_NEAR);
      }
      screenshotAnnotationTextFormatSize_ = size;
      screenshotAnnotationTextFormatScale_ = scale;
      screenshotAnnotationTextFormatFamily_ = family;
    }
    return screenshotAnnotationTextFormat_.Get();
  }
  ...
  bool EnsureFileIndexLoaded() {
    ...
    if (fileIndexLoadPending_ || fileIndexProjectionPending_) return false;
  ...
  void ApplyLoadedFileIndex(
    ...
    fileIndexLoadPending_ = false;
    fileIndexProjectionPending_ = true;
    fileIndexProjectionGeneration_ = generation;
    if (!fileSearchService_.UpdateStorageFilesAsync(std::move(entries),
                                                    generation)) {
      fileIndexProjectionPending_ = false;
      ReportBackgroundFailure(L"The file index projection worker is unavailable.");
    }
  }
  ...
  std::optional<std::vector<AppEntry>> DiscoverApps(std::stop_token stopToken,
    ...
    auto addUnique = [&](AppEntry entry) {
      ...
      if (!entry.targetPath.empty() && !IsHostExecutable(entry.targetPath) && entry.args.empty()) {
        candidateIdentities.push_back(L"target:" + canonicalPathKey(entry.targetPath));
      }
      ...
      if (!entry.isGame && !entry.launchTarget.empty()) {
        candidateIdentities.push_back(L"launch:" + canonicalPathKey(entry.launchTarget));
      }
      candidateIdentities.push_back(L"id:" + canonicalPathKey(entry.id));
      ...
      for (const auto& identity : candidateIdentities) {
        if (const auto found = identities.find(identity);
          ...
          if (feathercast::discovery::ShouldMergeApps(apps[found->second], entry)) {
      ...
      if (!foundIndex) {
        const std::wstring nameKey = L"name:" + NameKey(entry.name);
        if (const auto found = identities.find(nameKey); found != identities.end()) {
          if (feathercast::discovery::ShouldMergeApps(apps[found->second], entry)) {
            foundIndex = found->second;
          }
        }
      ...
      if (foundIndex) {
        feathercast::discovery::MergeAppEntries(apps[*foundIndex], std::move(entry));
        for (const auto& identity : candidateIdentities) {
          identities.emplace(identity, *foundIndex);
        }
        identities.emplace(L"name:" + NameKey(apps[*foundIndex].name), *foundIndex);
        return;
      }
      const size_t newIndex = apps.size();
      for (const auto& identity : candidateIdentities) {
        identities.emplace(identity, newIndex);
      }
      identities.emplace(L"name:" + NameKey(entry.name), newIndex);
  ...
  std::vector<std::filesystem::path> StartMenuShortcutPaths(std::stop_token stopToken,
    ...
    auto addDir = [&](const std::wstring& dirPath) {
      if (!dirPath.empty()) {
    ...
    addDir(KnownFolderPath(FOLDERID_CommonPrograms));
    addDir(KnownFolderPath(FOLDERID_Programs));
    addDir(KnownFolderPath(FOLDERID_PublicDesktop));
    addDir(KnownFolderPath(FOLDERID_Desktop));

    if (dirs.empty()) {
      wchar_t programData[MAX_PATH]{};
      wchar_t appData[MAX_PATH]{};
      wchar_t userProfile[MAX_PATH]{};
      wchar_t publicProfile[MAX_PATH]{};
      ...
      GetEnvironmentVariableW(L"PUBLIC", publicProfile, MAX_PATH);
      if (programData[0]) addDir(std::filesystem::path(programData) / L"Microsoft" / L"Windows" / L"Start Menu" / L"Programs");
      if (appData[0]) addDir(std::filesystem::path(appData) / L"Microsoft" / L"Windows" / L"Start Menu" / L"Programs");
      if (publicProfile[0]) addDir(std::filesystem::path(publicProfile) / L"Desktop");
      if (userProfile[0]) addDir(std::filesystem::path(userProfile) / L"Desktop");
    }
    ...
    for (const auto& dir : dirs) {
      ...
      for (std::filesystem::recursive_directory_iterator it(dir, std::filesystem::directory_options::skip_permission_denied, ec), end; it != end; it.increment(ec)) {
        if (ec) continue;
        if (it->is_regular_file(ec) && Lower(it->path().extension().wstring()) == L".lnk") {
          std::wstring canonical = Lower(it->path().lexically_normal().wstring());
  ...
  std::optional<AppEntry> ShortcutEntry(const std::filesystem::path& path) {
    const std::wstring baseName = CleanName(path.stem().wstring());
    if (baseName.empty() || ShouldSkipName(baseName)) return std::nullopt;
    const std::wstring name = DisambiguateShortcutName(baseName, path);
    ...
    entry.keywords = KeywordsFor(name, shortcut.target, shortcut.appUserModelId);
    return entry;
  }
      ...
      if (SUCCEEDED(folder->EnumObjects(nullptr, SHCONTF_FOLDERS | SHCONTF_NONFOLDERS, enumList.GetAddressOf()))) {
        ...
        while (enumList->Next(1, &child, &fetched) == S_OK) {
          ...
          if (!name.empty() && !appId.empty() && !ShouldSkipName(name)) {
            ...
            std::wstring targetPath = terminalAlias;
            if (targetPath.empty() && item) {
              PWSTR parsingPath = nullptr;
              if (SUCCEEDED(item->GetString(PKEY_Link_TargetParsingPath, &parsingPath)) && parsingPath) {
                targetPath = parsingPath;
                CoTaskMemFree(parsingPath);
              }
            }
            if (targetPath.empty() && item) {
              PWSTR fsPath = nullptr;
              if (SUCCEEDED(item->GetDisplayName(SIGDN_FILESYSPATH, &fsPath)) && fsPath) {
                targetPath = fsPath;
                CoTaskMemFree(fsPath);
              }
            }
            if (targetPath.empty() && appId.size() > 39 && appId.front() == L'{' && appId[37] == L'}' && appId[38] == L'\\') {
              const std::wstring guidStr = appId.substr(0, 38);
              GUID folderId{};
              if (SUCCEEDED(IIDFromString(guidStr.c_str(), &folderId))) {
                const std::wstring baseFolder = KnownFolderPath(folderId);
                if (!baseFolder.empty()) {
                  targetPath = (std::filesystem::path(baseFolder) / appId.substr(39)).wstring();
                }
              }
            }
            ...
            entry.systemEssential = IsSystemEssentialName(name);
            entry.keywords = KeywordsFor(name, targetPath.empty() ? terminalAlias : targetPath, appId);

            entry.source = appId.find(L"!") != std::wstring::npos ? L"appx" : L"start";
            entry.launchType = LaunchType::AppsFolder;
            entry.launchTarget = appId;
            // Keep the execution alias (e.g. wt.exe) or resolved target so the app can still be
  ...
  void RequestSearch() {
    ...
    if (!req.compactClear && !req.actionMode &&
      ...
      feathercast::files::FileQuery fileQuery;
      fileQuery.generation = req.generation;
      fileQuery.terms = req.query;
      fileQuery.limit = req.limit;
      fileQuery.contentEnabled = settings_.fileContentIndexEnabled;
      fileQuery.maxWorkers = req.maxWorkers;
      if (!fileSearchService_.Query(std::move(fileQuery))) {
  ...
  void UpdateBackgroundState() {
    const bool isForeground =
        ...
        captureUiState_.phase != feathercast::ui::CapturePhase::Idle;
    performanceGovernor_.SetInteractive(isForeground);
    extensions_.SetInteractive(isForeground);
    ...
    if (isForeground) {
      ...
      fileIndexLoadPending_ = false;
      fileIndexProjectionPending_ = false;
      ++fileIndexProjectionGeneration_;
      KillTimer(hwnd_, TIMER_FILE_SNAPSHOT);
      ...
      fileSearchService_.UpdateFilesAsync({}, fileIndexProjectionGeneration_);
      hits_.clear();
  ...
  void RetryOverlayActivation() {
    ...
    if (WinShortcutTransitionPending()) return;
  ...
  void FinishHideOverlay(PendingOverlayClose close) {
    ...
    overlayOpacity_.Snap(1.0);
    RECT currentBounds{};
    if (GetWindowRect(hwnd_, &currentBounds)) {
      SnapWindowBounds(overlayBounds_, currentBounds);
    }
  ...
  void FinishHideSettings() {
    ...
    settingsOpacity_.Snap(1.0);
    RECT currentBounds{};
    if (GetWindowRect(settingsHwnd_, &currentBounds)) {
      SnapWindowBounds(settingsBounds_, currentBounds);
    }
  ...
  static int DipToPixels(float dip, float scale) {
    return std::max(1, feathercast::screenshot::DipToPixel(dip, scale));
  }

  static float PixelsToDip(int pixels, float scale) {
    return feathercast::screenshot::PixelToDip(pixels, scale);
  }
  ...
  int CurrentHeight() const {
    if (confirmation_) return 300;
    const int baseHeight = settings_.compactMode ? COMPACT_BASE_HEIGHT
                                                 : (COMPACT_BASE_HEIGHT + 40);
  ...
  void UpdateSelectionAnimation(double deltaTime) {
    ...
    // Scrolling moves the row under the pill mid-flight; retarget rather than
    // restart so the pill never jumps.
    selectionSpring_.Retarget(*targetY);
    selectionSpring_.Update(deltaTime);
    visualSelectedY_ = static_cast<float>(selectionSpring_.Value());
    animatingSelection_ = selectionSpring_.Active();
  }
  ...
  void OnAnimationFrame() {
    ...
    if (animating_ == false && iconPresentationDeferred_) {
      ...
      iconPresentationDeferred_ = HasPendingVisibleIcons();
      ScheduleIconPromotion();
    }
  ...
  void Paint() {
    ...
    const UINT clientWidth = std::max<UINT>(1, clientRc.right - clientRc.left);
    const UINT clientHeight = std::max<UINT>(1, clientRc.bottom - clientRc.top);
    DXGI_SWAP_CHAIN_DESC1 swapDesc{};
    if (overlaySurface_.swapChain &&
        SUCCEEDED(overlaySurface_.swapChain->GetDesc1(&swapDesc))) {
      if (swapDesc.Width != clientWidth || swapDesc.Height != clientHeight) {
        ResizeGlassSurface(overlaySurface_, hwnd_, clientWidth, clientHeight);
      }
    }
    {
  ...
  void PaintSettings() {
    ...
    RECT settingsRc{};
    GetClientRect(settingsHwnd_, &settingsRc);
    const UINT settingsWidth = std::max<UINT>(1, settingsRc.right - settingsRc.left);
    const UINT settingsHeight = std::max<UINT>(1, settingsRc.bottom - settingsRc.top);
    DXGI_SWAP_CHAIN_DESC1 settingsSwapDesc{};
    if (settingsSurface_.swapChain &&
        SUCCEEDED(settingsSurface_.swapChain->GetDesc1(&settingsSwapDesc))) {
      if (settingsSwapDesc.Width != settingsWidth || settingsSwapDesc.Height != settingsHeight) {
        ResizeGlassSurface(settingsSurface_, settingsHwnd_, settingsWidth, settingsHeight);
      }
    }
    {
  ...
  void PaintVolumeControl() {
    ...
    RECT volumeRc{};
    GetClientRect(volumeHwnd_, &volumeRc);
    const UINT volumeWidth = std::max<UINT>(1, volumeRc.right - volumeRc.left);
    const UINT volumeHeight = std::max<UINT>(1, volumeRc.bottom - volumeRc.top);
    DXGI_SWAP_CHAIN_DESC1 volumeSwapDesc{};
    if (volumeSurface_.swapChain &&
        SUCCEEDED(volumeSurface_.swapChain->GetDesc1(&volumeSwapDesc))) {
      if (volumeSwapDesc.Width != volumeWidth || volumeSwapDesc.Height != volumeHeight) {
        ResizeGlassSurface(volumeSurface_, volumeHwnd_, volumeWidth, volumeHeight);
      }
    }
    {
  ...
  void DrawPreviewPane(RectF rect) {
    ...
    if (!performanceGovernor_.Policy().allowRichPreview) {
      DrawTextBlock(L"Rich preview deferred while the UI is busy.",
                    {left, rect.top + 128.0f, right, rect.top + 164.0f},
                    bodyFormat_.Get(), D2DColor(theme_.textMuted));
      return;
    }
    ...
    if (preview.kind == feathercast::preview::Kind::Text) {
      ...
      if (previewBitmap_) {
        ...
        DrawTextBlock(L"Preparing image preview...",
                      {left, contentTop + 12.0f, right, contentTop + 44.0f},
                      bodyFormat_.Get(), D2DColor(theme_.textMuted));
      }
    }
  }
  ...
  void DrawSearch(bool drawBackground) {
    ...
    const bool richPreviewAllowed =
        performanceGovernor_.Policy().allowRichPreview;
    const DisplayItem* detailItem =
        previewOpen_ || !richPreviewAllowed ? nullptr : SelectedMarkdownDetailItem();
  ...
  void DrawSelectionPill(float width) {
    ...
    if (animatingSelection_) {
      const float lead = std::clamp(
          static_cast<float>(std::abs(selectionSpring_.Velocity())) * 0.012f,
          0.0f, kSelectionLeadStretch);
      if (selectionSpring_.Velocity() > 0.0) {
        rect.bottom += lead;
      } else {
        rect.top -= lead;
      }
  ...
  const std::vector<MarkdownRenderLine>& MarkdownLinesFor(
      const DisplayItem& item) {
    const std::wstring key = item.Key();
    if (const auto found = markdownCache_.find(key);
        found != markdownCache_.end()) {
      return found->second;
    }

    constexpr std::size_t kMarkdownInputCap = 64 * 1024;
    constexpr std::size_t kMarkdownLineCap = 512;
    std::vector<MarkdownRenderLine> parsed;
    std::wistringstream lines(
        item.extension.detailBody.substr(0, kMarkdownInputCap));
    std::wstring line;
    bool codeBlock = false;
    while (std::getline(lines, line)) {
      if (parsed.size() >= kMarkdownLineCap) break;
      if (!line.empty() && line.back() == L'\r') line.pop_back();
      std::wstring trimmed = Trim(line);
      if (StartsWith(trimmed, L"```")) {
        codeBlock = !codeBlock;
        parsed.push_back({L"", false, false, codeBlock ? 4.0f : 6.0f});
        continue;
      }
      if (trimmed.empty()) {
        parsed.push_back({L"", false, false, 8.0f});
        continue;
      }
      if (codeBlock) {
        parsed.push_back({std::move(trimmed), true, false, 23.0f});
        continue;
      }
      ...
      parsed.push_back(
          {std::move(trimmed), false, heading, advance});
    }

    if (markdownCache_.size() >= kMarkdownCacheCap) {
      markdownCache_.erase(markdownCache_.begin());
    }
    const auto [inserted, _] =
        markdownCache_.emplace(key, std::move(parsed));
  ...
  void DrawMarkdownDetail(const DisplayItem& item, RectF rect) {
    ...
    for (const auto& markdownLine : MarkdownLinesFor(item)) {
      if (y >= rect.bottom - 12.0f) break;
      if (markdownLine.text.empty()) {
        y += markdownLine.advance;
        continue;
      }

      if (markdownLine.codeBlock) {
        FillRound({left - 4.0f, y - 1.0f, right + 4.0f, y + 20.0f}, 4.0f, D2DColor(theme_.iconTile));
        DrawTextBlock(markdownLine.text, {left, y + 1.0f, right, y + 19.0f}, subFormat_.Get(), D2DColor(theme_.textPrimary));
        y += 23.0f;
        continue;
      }

      const bool heading = markdownLine.heading;
      DrawTextBlock(markdownLine.text, {left, y, right,
                                        y + markdownLine.advance},
                    heading ? titleFormat_.Get() : bodyFormat_.Get(),
                    heading ? D2DColor(theme_.textPrimary)
                            : (StartsWith(markdownLine.text, L"- ")
                                   ? D2DColor(theme_.textPrimary)
                                   : D2DColor(theme_.textMuted)));
      y += markdownLine.advance;
    }
  ...
  struct ResultRenderData {
    std::wstring name;
    std::wstring source;
    std::wstring iconKey;
  };

  const ResultRenderData& ResultRenderDataFor(const DisplayItem& item) {
    const std::wstring key = item.Key();
    if (const auto found = resultRenderCache_.find(key);
        found != resultRenderCache_.end()) {
      return found->second;
    }
    if (resultRenderCache_.size() >= kResultRenderCacheCap) {
      resultRenderCache_.erase(resultRenderCache_.begin());
    }
    const auto [inserted, _] = resultRenderCache_.emplace(
        key, ResultRenderData{item.Name(), SourceLabel(item), item.IconKey()});
    return inserted->second;
  }
  ...
  std::wstring SourceLabel(const DisplayItem& item) const {
    ...
    if (item.timerRequest || !item.settingId.empty()) {
      return L"Open · " + item.commandDetail;
    }
    if (item.isCapability) {
      std::wstring verb = L"Open";
      switch (item.capability.action.kind) {
        case CapabilityActionKind::SeedQuery: verb = L"Try"; break;
        case CapabilityActionKind::OpenBrowse: verb = L"Browse"; break;
        case CapabilityActionKind::OpenSettings: verb = L"Open"; break;
        case CapabilityActionKind::RunCommand: verb = L"Run"; break;
      }
      return verb + L" · " + item.capability.summary +
    ...
    if (item.isExtension) return L"Run · " + item.commandDetail;
    if (item.isSnippet) return L"Paste · Snippet · " + item.snippet.keyword;
    if (item.isClipboard) {
      return item.clipboard.pinned ? L"Paste · Favorite clipboard"
                                   : L"Paste · Clipboard history";
    }
    if (item.isRunCommand) return L"Run · " + item.commandDetail;
    if (item.isSymbol) return L"Paste · Symbol";
    if (item.utility) return L"Copy · Local utility · " + item.utility->value;
    if (item.isCommand) return L"Run · " + item.commandDetail;
    if (item.isAction) return L"Apply · " + item.commandDetail;
    if (item.isWindow) return item.window.processName.empty()
                                ? L"Switch · Open window"
                                : L"Switch · Open window · " + item.window.processName;
    if (item.app.isGame) {
      return item.app.gameProvider.empty()
                 ? L"Open · Installed game"
                 : L"Open · " + item.app.gameProvider + L" game";
    }
    ...
    if (item.app.source == L"file") {
      if (item.app.fileContentMatch) {
        return item.app.path.empty() ? L"Find · Content match"
                                     : L"Find · Content match · " + item.app.path;
      }
      return item.app.path.empty() ? L"Open · File or folder"
                                   : L"Open · " + item.app.path;
    }
    if (item.app.source == L"system-folder") {
  ...
  void DrawSettings() {
    ...
    std::wstring displayedFilter = settingsState_.filter;
    if (!settingsState_.filterImeComposition.empty()) {
      displayedFilter.insert(
          std::min(settingsState_.filterCaret, displayedFilter.size()),
    ...
    const float viewportBottom = height - kSettHelpHeight;
    const float maxScroll =
        std::max(0.0f, static_cast<float>(SettingsPageContentHeight()) -
                            viewportBottom);
    ...
    if (maxScroll > 0.0f) {
      ...
      const float trackBottom = viewportBottom - 8.0f;
      const float trackH = trackBottom - trackTop;
      const float contentH = static_cast<float>(SettingsPageContentHeight());
      const float thumbH = std::max(36.0f, trackH * (viewportBottom / contentH));
      const float thumbY =
    ...
    auto helpBorder = Brush(D2DColor(theme_.divider));
    activeRT_->DrawLine(D2D1::Point2F(kSettSidebarWidth, height - kSettHelpHeight),
                        D2D1::Point2F(width, height - kSettHelpHeight),
                        helpBorder.Get(), 1.0f);
    DrawTextBlock(L"Tab navigate · Enter apply · Esc close",
                  {kSettSidebarWidth + kSettContentInset,
                   height - kSettHelpHeight + 8.0f, width - 18.0f,
                   height - 8.0f},
                  footerRightFormat_.Get(), D2DColor(theme_.textMuted));
  }
  ...
  bool PromotePreviewBitmap() {
    if (previewBitmap_ || !previewResult_ ||
        previewResult_->kind != feathercast::preview::Kind::Image ||
        previewResult_->pixels.empty() || !overlaySurface_.dc) {
      return previewBitmap_ != nullptr;
    }
    const auto& preview = *previewResult_;
    if (preview.width == 0 || preview.height == 0 || preview.stride == 0) {
      return false;
    }
    ...
    if (FAILED(overlaySurface_.dc->CreateBitmap(
            ...
            preview.stride, properties, bitmap.GetAddressOf()))) {
      return false;
    }
    previewBitmap_ = std::move(bitmap);
    return true;
  }

  bool PromoteScreenshotDraftBitmap() {
    if (screenshotDraftBitmap_ || !screenshotDraft_ ||
        !screenshotDraft_->Valid() || !captureSelectorSurface_.dc) {
      return screenshotDraftBitmap_ != nullptr;
    }
    const auto& draft = *screenshotDraft_;
    const D2D1_BITMAP_PROPERTIES properties = D2D1::BitmapProperties(
        D2D1::PixelFormat(DXGI_FORMAT_B8G8R8A8_UNORM,
                          D2D1_ALPHA_MODE_PREMULTIPLIED));
    ComPtr<ID2D1Bitmap> bitmap;
    if (FAILED(captureSelectorSurface_.dc->CreateBitmap(
            D2D1::SizeU(draft.width, draft.height), draft.pixels->data(),
            draft.stride, properties, bitmap.GetAddressOf()))) {
      return false;
    }
    screenshotDraftBitmap_ = std::move(bitmap);
    return true;
  }
  ...
  std::size_t PromotePendingVisibleIcons(std::size_t maxCount) {
    if (!overlaySurface_.dc || maxCount == 0) return 0;
    std::size_t promoted = 0;
    for (const auto& key : visibleIconKeys_) {
      if (promoted >= maxCount) break;
  ...
  bool HasPendingVisibleIcons() const {
    return std::any_of(
        visibleIconKeys_.begin(), visibleIconKeys_.end(), [&](const auto& key) {
          return iconBitmaps_.find(key) == iconBitmaps_.end() &&
                 pendingDecodedIcons_.contains(key);
        });
  }

  void ScheduleIconPromotion() {
    if (!visible_ || !HasPendingVisibleIcons() || iconPromotionTimerArmed_) {
      return;
    }
    if (SetTimer(hwnd_, TIMER_ICON_PROMOTE,
                 performanceGovernor_.Policy().tier ==
                         feathercast::performance::QualityTier::Critical
                     ? 33
                     : 16,
                 nullptr) != 0) {
      iconPromotionTimerArmed_ = true;
    }
  }
  ...
  ComPtr<ID2D1Bitmap> IconBitmap(const std::wstring& key) {
    ...
    // D2D bitmap creation is intentionally never performed from WM_PAINT.
    // The bounded promotion timer owns that work so one large icon batch
    // cannot turn a paint into an unbounded upload spike.
    QueueIcon(key);
    return nullptr;
  }
  ...
  void StartIconWorkers() {
    const auto policy = performanceGovernor_.Policy();
    const size_t workers = policy.tier ==
                                   feathercast::performance::QualityTier::Full
                               ? 2
                               : 1;
  ...
  void OnKeyDown(UINT vk) {
    ...
    if (view_ == View::Search && control && vk == VK_OEM_COMMA) {
  ...
  void SelectSettingsCategory(SettingsCategory category,
    ...
    settingsFocusIndex_ = SettingsCategoryFocusIndex(category);
  ...
  void OnPointerDown(HWND owner, float x, float y) {
    ...
    if (owner == settingsHwnd_) {
      ...
      if (hit->type == HitType::SettingsFilter) {
        SetFocus(settingsHwnd_);
      }
  ...
  bool SettingsFilterFocused() const {
    const auto order = SettingsFocusOrder();
    return settingsFocusIndex_ >= 0 &&
           settingsFocusIndex_ < static_cast<int>(order.size()) &&
           order[static_cast<size_t>(settingsFocusIndex_)] ==
               HitType::SettingsFilter;
  }
  ...
  std::optional<std::pair<size_t, size_t>> SettingsFilterSelectionRange() const {
    if (!settingsState_.filterSelectionAnchor ||
        *settingsState_.filterSelectionAnchor == settingsState_.filterCaret) {
      return std::nullopt;
    }
    return std::pair{
        std::min(*settingsState_.filterSelectionAnchor,
                 settingsState_.filterCaret),
        std::max(*settingsState_.filterSelectionAnchor,
                 settingsState_.filterCaret)};
  }

  void MoveSettingsFilterCaret(size_t next, bool extendSelection) {
    ClampSettingsFilterCaret();
    if (extendSelection) {
      if (!settingsState_.filterSelectionAnchor) {
        settingsState_.filterSelectionAnchor = settingsState_.filterCaret;
  ...
  void ClearSettingsFilterSelection() {
    if (const auto range = SettingsFilterSelectionRange()) {
      settingsState_.filter.erase(range->first, range->second - range->first);
      settingsState_.filterCaret = range->first;
      settingsState_.filterSelectionAnchor.reset();
    }
  }

  void FocusSettingsFilterMatch() {
    if (!settingsState_.searchTarget) return;
    const auto order = SettingsFocusOrder();
    const auto found = std::find(order.begin(), order.end(),
                                 *settingsState_.searchTarget);
  ...
  void ApplySettingsFilter() {
    const std::wstring query = Trim(settingsState_.filter);
    if (query.empty()) {
    ...
    const auto matches = feathercast::settings_catalog::Search(query);
    if (matches.empty()) {
      settingsState_.searchTarget.reset();
      settingsFocusIndex_ = 0;
      return;
    ...
    const std::wstring filter = settingsState_.filter;
    const size_t caret = settingsState_.filterCaret;
    const auto selection = settingsState_.filterSelectionAnchor;
    if (settingsCategory_ != match->category) {
      SelectSettingsCategory(match->category, false);
      settingsState_.filter = filter;
      settingsState_.filterCaret = caret;
      settingsState_.filterSelectionAnchor = selection;
    }
    settingsState_.searchTarget = match->hit;
    const auto order = SettingsFocusOrder();
    const auto found = std::find(order.begin(), order.end(), match->hit);
    if (found != order.end()) {
  ...
  void InsertSettingsFilterText(const std::wstring& text) {
    if (!SettingsFilterFocused() || text.empty()) return;
    ClampSettingsFilterCaret();
    ClearSettingsFilterSelection();
  ...
  void OnSettingsChar(wchar_t ch) {
    if (settingsClosing_ || recording_ || !SettingsFilterFocused()) return;
    if (ch >= 32 && ch != 127) InsertSettingsFilterText(std::wstring(1, ch));
  }

  void HandleSettingsFilterKeyDown(UINT vk) {
    const bool control = ModifierPressed(VK_CONTROL);
    const bool shift = ModifierPressed(VK_SHIFT);
    if (control && vk == 'A') {
    ...
    if (vk == VK_RETURN || vk == VK_DOWN || vk == VK_UP) {
      FocusSettingsFilterMatch();
      return;
    }
    if (vk == VK_HOME) {
      MoveSettingsFilterCaret(0, shift);
      return;
    }
    if (vk == VK_END) {
      MoveSettingsFilterCaret(settingsState_.filter.size(), shift);
      return;
    }
    if (vk == VK_LEFT) {
      const size_t next = settingsState_.filterCaret == 0
                              ? 0
                              : (control
                                     ? feathercast::text_edit::PreviousWord(
                                           settingsState_.filter,
                                           settingsState_.filterCaret)
                                     : feathercast::text_edit::PreviousCodePoint(
                                           settingsState_.filter,
                                           settingsState_.filterCaret));
      MoveSettingsFilterCaret(next, shift);
      return;
    }
    if (vk == VK_RIGHT) {
      const size_t next = settingsState_.filterCaret >= settingsState_.filter.size()
                              ? settingsState_.filter.size()
                              : (control
                                     ? feathercast::text_edit::NextWord(
                                           settingsState_.filter,
                                           settingsState_.filterCaret)
                                     : feathercast::text_edit::NextCodePoint(
                                           settingsState_.filter,
                                           settingsState_.filterCaret));
      MoveSettingsFilterCaret(next, shift);
      return;
    ...
    if (vk == VK_BACK) {
      if (SettingsFilterSelectionRange()) {
        ClearSettingsFilterSelection();
      } else if (settingsState_.filterCaret > 0) {
        const size_t previous = control
                                    ? feathercast::text_edit::PreviousWord(
                                          settingsState_.filter,
                                          settingsState_.filterCaret)
                                    : feathercast::text_edit::PreviousCodePoint(
                                          settingsState_.filter,
                                          settingsState_.filterCaret);
        settingsState_.filter.erase(previous,
                                     settingsState_.filterCaret - previous);
        settingsState_.filterCaret = previous;
      }
      settingsState_.filterSelectionAnchor.reset();
      ApplySettingsFilter();
    ...
    if (vk == VK_DELETE) {
      if (SettingsFilterSelectionRange()) {
        ClearSettingsFilterSelection();
      } else if (settingsState_.filterCaret < settingsState_.filter.size()) {
        const size_t next = control
                                ? feathercast::text_edit::NextWord(
                                      settingsState_.filter,
                                      settingsState_.filterCaret)
                                : feathercast::text_edit::NextCodePoint(
                                      settingsState_.filter,
                                      settingsState_.filterCaret);
        settingsState_.filter.erase(settingsState_.filterCaret,
                                     next - settingsState_.filterCaret);
      }
      settingsState_.filterSelectionAnchor.reset();
      ApplySettingsFilter();
      settingsFocusIndex_ = 0;
  ...
  void OnSettingsFilterImeComposition(LPARAM flags) {
    if (!SettingsFilterFocused()) return;
    HIMC context = ImmGetContext(settingsHwnd_);
    if (!context) return;
    ScopeExit releaseContext([&] { ImmReleaseContext(settingsHwnd_, context); });
    ...
    if ((flags & GCS_RESULTSTR) != 0) {
      ...
      settingsState_.filterImeComposition.clear();
      InsertSettingsFilterText(result);
    } else if ((flags & GCS_COMPSTR) != 0) {
      settingsState_.filterImeComposition = readComposition(GCS_COMPSTR);
    }
    ...
    candidate.ptCurrentPos.x = static_cast<LONG>(
        (filterRect.left + 34.0f +
  ...
  void HandleSettingsKeyDown(UINT vk) {
    ...
    if (focusedType == HitType::SettingsFilter) {
      HandleSettingsFilterKeyDown(vk);
      return;
    }
  ...
  static bool IsClipboardExcluded(
      HWND foreground, const std::vector<std::wstring>& exclusions) {
  ...
  void HandleClipboardText(std::wstring text) {
    ...
    if (!persistence_.StoreClipboard(std::move(text), preview, capturedAt,
  ...
  void OnClipboardUpdate() {
    ...
    if (!launchExecutor_.Submit(
      ...
      clipboardUpdatePending_.store(false, std::memory_order_release);
      ReportPersistenceFailure(L"The clipboard worker is unavailable.");
    }
  }
  ...
  void ClearIconCache() {
    if (iconCacheClearPending_.exchange(true, std::memory_order_acq_rel)) {
      return;
    }
    iconResolver_.ClearPending();
    ClearIconBitmaps();
    const auto cacheDirectory = IconCacheDir();
    if (!launchExecutor_.Submit(
            [this, cacheDirectory](std::stop_token stopToken) {
              if (stopToken.stop_requested()) return;
              // Stopping a shell/icon worker can wait for an in-flight COM
              // decode. Keep that wait off the message thread as well as the
              // filesystem mutation below.
              StopIconThreads();
              if (stopToken.stop_requested()) return;
              std::error_code ec;
              std::filesystem::remove_all(cacheDirectory, ec);
              if (!ec) std::filesystem::create_directories(cacheDirectory, ec);
              iconCacheClearPending_.store(false, std::memory_order_release);
              if (stopToken.stop_requested() || stopThreads_ || !hwnd_) return;
              if (ec) {
                PostMessageW(hwnd_, WM_BACKGROUND_TASK_ERROR, 0, 0);
              } else {
                PostMessageW(hwnd_, WM_REBUILD_RESULTS, 0, 0);
      ...
      iconCacheClearPending_.store(false, std::memory_order_release);
      ReportBackgroundFailure(L"The icon-cache worker is unavailable.");
      return;
    }
    SetSettingsStatus(StatusSeverity::Progress, L"Clearing icon cache...");
  ...
  feathercast::runtime::UiEventQueue<feathercast::persistence::Event>
      persistenceEvents_;
  feathercast::runtime::UiEventQueue<SnippetSaveCompleted>
      snippetSaveEvents_;
  ...
  feathercast::runtime::UiEventQueue<feathercast::files::PreparedFileIndex>
      fileProjectionEvents_;
  ...
  feathercast::screenshot::Rect screenshotEffectPreviewBounds_;
  std::optional<feathercast::screenshot::Rect>
      screenshotEffectPreviewSelection_;
  std::uint64_t screenshotAnnotationRevision_ = 1;
  std::uint64_t screenshotEffectPreviewRevision_ = 0;
  ...
  HWND screenshotTextEditHwnd_ = nullptr;
  bool screenshotCaptureUiExcluded_ = false;
  ...
  std::atomic<bool> iconCacheClearPending_ = false;
  std::atomic<bool> clipboardUpdatePending_ = false;
  ...
  bool fileIndexProjectionPending_ = false;
  std::uint64_t fileIndexProjectionGeneration_ = 0;
  ...
  ComPtr<IDWriteTextFormat> screenshotAnnotationTextFormat_;
  float screenshotAnnotationTextFormatSize_ = 0.0f;
  float screenshotAnnotationTextFormatScale_ = 0.0f;
  std::wstring screenshotAnnotationTextFormatFamily_;
};
